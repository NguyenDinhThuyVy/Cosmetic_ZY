...
export default function ProductSale() {
  const [numProducts, setNumProducts] = useState(14)

  const handleNumProductsChange = (event) => {
    const newNumProducts = parseInt(event.target.value)
    setNumProducts(newNumProducts)
  }

  const calculateColor = (numProducts) => {
    const normalizedNumProducts = Math.min(100, numProducts) / 100
    return chroma.scale(['red', 'orange', 'yellow', 'green', 'blue']).get(normalizedNumProducts)
  }
  return (
      ...
      <div className=' bg-[#ffffff] flex flex-col justify-between gap-5 items-center rounded-2xl'>
        ...
        <div className='flex flex-col items-center'>
          <input
            ...
            className='mt-4 w-full border border-gray-300 rounded-md px-2 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500'
          />
          <div
            style={{
              width: `${numProducts}%`,
              height: '8px',
              borderRadius: '50%',
              background: calculateColor(numProducts),
