...
function TableDataDelete() {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (selectedRecord) {
        await adminApi.retoreProduct(selectedRecord._id :any, values.quantity)
    ...
    return (
      <>
        <Table
          ...
          dataSource={products}
        />
        <Modal title='' visible={isModalVisible} onCancel={handleCancel} {...formItemLayout} width={380}>
          ...
          <div className='flex flex-col  gap-4 border border-gray-200 rounded-lg w-full px-4 pt-2  font  '>
            ...
            <p>Bạn vui lòng nhập số lượng sản phẩm hiện có để khôi phục mặt hàng này nhé !</p>
            <Form form={form}>
