...
const FormBrandEdit: React.FC<CollectionCreateFormProps> = ({ brandId, onClose, onUpdateSuccess }) => {
  ...
  type FormData = Pick<BrandSchema, 'name' | 'address' | 'phone' | 'date_of_birth' | 'avatar'>
  const profileSchema = ba.pick(['name', 'address', 'phone', 'date_of_birth', 'avatar'])
  useEffect(() => {
