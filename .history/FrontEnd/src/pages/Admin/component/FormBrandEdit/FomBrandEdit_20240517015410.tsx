...
const FormBrandEdit: React.FC<CollectionCreateFormProps> = ({ brandId, onClose, onUpdateSuccess }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const hasUploadedNewImage = form.getFieldValue('image') !== null
      // Nếu không có ảnh mới được tải lên, không cần cập nhật giá trị ảnh
      if (!hasUploadedNewImage) {
        delete values.image // Xóa trường image khỏi values để tránh gửi giá trị null lên server
      }
