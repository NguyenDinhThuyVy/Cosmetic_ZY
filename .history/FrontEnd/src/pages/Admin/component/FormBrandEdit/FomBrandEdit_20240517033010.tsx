  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const { data } = brandDataResponse.data

        // Nếu image không tồn tại, set giá trị mặc định cho tất cả các trường
        if (!data.image) {
          form.setFieldsValue(data)
        } else {
          // Nếu image tồn tại, set giá trị mặc định cho tất cả các trường trừ image
