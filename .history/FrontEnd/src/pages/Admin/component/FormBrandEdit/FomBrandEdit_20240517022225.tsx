...
const FormBrandEdit: React.FC<CollectionCreateFormProps> = ({ brandId, onClose, onUpdateSuccess }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Nếu không có ảnh mới được tải lên, loại bỏ trường image khỏi values
      if (values.image === image) {
        delete values.image
      }

      const response = await adminApi.updateBrand(brandId, values)
  ...
  const customRequest = async ({ file, onSuccess, onError }) => {
    ...
    try {
      ...
      setimage(responseData.data) // Cập nhật state và form field
      form.setFieldsValue({ image: responseData.data })
    } catch (error) {
      onError(error)
    }
  }
