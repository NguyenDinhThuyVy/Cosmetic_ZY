...
const FormBrandEdit: React.FC<CollectionCreateFormProps> = ({ brandId, onClose, onUpdateSuccess }) => {
  ...
  const [initialImage, setInitialImage] = useState<string | undefined>(brandData.data.image)
  ...
  const handleSave = async () => {
    try {
      ...
      const formValuesChanged = Object.keys(values).some((key) => values[key] !== brandData[key])

      // Only upload the image if it has changed or if there was no initial image
      const formData = new FormData()
      formData.append('image', info.file.originFileObj || '')
      if ((values.image !== initialImage || !initialImage) && formValuesChanged) {
        const uploadRes = await adminApi.uploadBrandImage(formData)
        values.image = uploadRes.data.data
      }
