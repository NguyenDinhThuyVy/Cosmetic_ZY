...
const FormBrandEdit: React.FC<CollectionCreateFormProps> = ({ brandId, onClose, onUpdateSuccess }) => {
  ...
  const uploadImageMutaion = useMutation({
    mutationFn: adminApi.uploadBrandImage
  })
  ...
  Để chỉ gọi hàm handleUploadChange khi người dùng thực sự tải lên một tập tin mới và không gọi khi họ không tải lên, bạn có thể thêm điều kiện để kiểm tra xem info.file có tồn tại hay không. Nếu info.file không tồn tại, có nghĩa là người dùng không thực hiện hành động tải lên, và bạn có thể tránh gọi hàm handleUploadChange trong trường hợp này.
  ...
  const handleUploadChange = async (info: any) => {
    // Kiểm tra xem info.file có tồn tại không trước khi tiếp tục
    if (info.file) {
