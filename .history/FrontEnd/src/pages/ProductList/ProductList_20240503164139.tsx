...
export default function ProductList() {
  ...
  const { data: productsDataSold } = useQuery({
    queryKey: ['products', queryConfig],
    queryFn: () => {
      return productApi.getProducts({ ...(queryConfig as ProductListConfig), sort_by: sortBy.sold })
    },
    keepPreviousData: true,
    staleTime: 3 * 60 * 1000
  })
  const handleSort = (sortByValue: Exclude<ProductListConfig['sort_by'], undefined>) => {
    const sortedData = productApi.getProducts({ ...(queryConfig as ProductListConfig), sort_by: sortByValue })
  ...
  const { data: productsDataSold } = useQuery({
    queryKey: ['products', queryConfig],
    queryFn: () => {
      return sortedData || productApi.getProducts({ ...(queryConfig as ProductListConfig), sort_by: sortBy.sold })
    },
    keepPreviousData: true,
    staleTime: 3 * 60 * 1000
  })
