...
export default function ProductList() {
  ...
  const { data: productsDataSort } = useQuery({
    queryKey: ['products', queryConfig],
    queryFn: async () => {
      // Thực hiện điều hướng và trả về promise
      await navigate({
        pathname: path.home,
      ...
      // Trả về dữ liệu không gì để tránh lỗi từ useQuery
      return null
    },
    keepPreviousData: true,
    staleTime: 3 * 60 * 1000
  ...
  const { data: productsDataSold, isLoading } = useQuery({
    queryKey: ['products', queryConfig],
    queryFn: async () => {
      const products = await productApi.getProducts(queryConfig as ProductListConfig)
      // Thực hiện sắp xếp dữ liệu ở đây dựa trên giá trị của sortByValue
      const sortedProducts = <products className="sort"></products>((a, b) => {
        // Đảm bảo a và b đều có giá trị của trường bạn muốn sắp xếp
        // Ví dụ: sắp xếp theo giá
    ...
    <div className='h-full flex flex-col font '>
      <div className=' min-h-32'>
        ...
        {productsDataSold && (
          <div className='my-10 mx-28 w-5/6'>
            <div className='flex flex-col gap-2 items-center justify-center '>
              ...
              <ProductTop data={productsDataSold.data.data.products}></ProductTop>
              <Link
                to={path.filterProduct}
        ...
        {productsDataSort && (
          <div className='my-10 mx-28 w-5/6'>
            <div className='flex flex-col gap-2 items-center justify-center '>
              <div className='text-center text-2xl font-semibold uppercase mt-6 '>top sản phẩm mới nhất</div>
              <ProductTop data={productsDataSort.data.data.products}></ProductTop>
              <Link
                to={path.filterProduct}
                className='text-gray-600 bg-white border-2 border-gray-600 rounded-full px-6 py-2 font-bold hover:border-rose-700 hover:text-rose-700'
                onClick={() => {
