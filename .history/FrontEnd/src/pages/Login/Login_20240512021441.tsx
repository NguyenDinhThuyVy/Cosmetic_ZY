...
export default function Login() {
  ...
  const handleLoginGoogle = useGoogleLogin({
    onSuccess: async (data) => {
      ...
      toast.success('Đăng nhập thành công!', {
        autoClose: 1500 // Tự động đóng thông báo sau 2 giây
      })
      onClose: () => {
        // After the toast message closes
        navigate('/') // Navigate to the homepage or another route
        setTimeout(() => {
          window.location.reload() // Reload the window to refresh the state
        }, 0) // Immediately after navigation or after a small delay if needed
      }
    }
  })
  return (
