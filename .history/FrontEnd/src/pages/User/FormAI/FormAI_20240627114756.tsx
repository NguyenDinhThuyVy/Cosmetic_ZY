...
export default function FormAI() {
  ...
  const getCategoryStep = (category: keyof CategoryMap, productCount: keyof typeof any): number | '' => {
    ...
    return categories[productCount.toString()]?.[category] || ''
  }
  const step1 = getCategoryStep('6630b96d64d8fa096524aa32', '3') // Returns 1
  const step2 = getCategoryStep('6630b95d64d8fa096524aa31', '5') // Returns 2

  console.log(step1) // Output: 1
  return (
        ...
        <div className='flex flex-col border-2 border-gray-100 rounded-lg p-4 font relative'>
          ...
          <table className='table-auto w-full mt-5 text-[12px] border-2 border-gray-100 rounded-lg p-4'>
            ...
            {products.map((product, index) => {
              const step = getCategoryStep(product?.category as keyof CategoryMap, products.length.toString())
