...
type FormData = {
  ...
  avatar?: string
  date_of_birth?: Date
  type: number // Define 'type' as a number
}
...
const profileSchema = userSchema.pick(['name', 'address', 'phone', 'date_of_birth', 'avatar']).shape({
...
export default function Profile() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setValue('type', profile.type || 0) // Ensure 'type' is set and is a number
    }
  }, [profile, setValue])
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      toast.success(res.data.message, {
        autoClose: 1300 // Display for 1.3 seconds
      })
    } catch (error) {
      if (isAxiosUnprocessableEntityError<ErrorResponse<FormDataError>>(error)) {
        ...
        if (formError) {
          Object.keys(formError).forEach((key) => {
            const errorMessage = String(formError[key as keyof FormDataError]) // Convert error message to string
