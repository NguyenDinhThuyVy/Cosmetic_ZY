import { CheckboxProps, ConfigProvider, Radio, RadioChangeEvent } from 'antd'
...
export default function CheckBoxCategory({ queryConfig, categories }: Props) {
  ...
  const renderCheckboxes = () => {
    ...
    const checkboxes = categories.map((categoryItem, index) => (
      <Radio key={index} onChange={onChange} value={categoryItem._id}>
        {categoryItem.name}
      </Radio>
    ))
    ...
    <div className='flex flex-col gap-5 scrollable-container' style={{ maxHeight: '310px', overflowY: 'auto' }}>
      <ConfigProvider theme={myTheme}>
        <div className='flex flex-col gap-5'>
          <Radio.Group value={selectedValue}></Radio.Group>
        </div>
      </ConfigProvider>
      {!expanded && (
        <button
