    ...
    if (paymentData && Array.isArray(paymentData.data.data) && paymentData.data.data.length > 0) {
      paymentData.data.data.forEach((payment) => {
        if (!viewedPayments.includes(payment._id)) {
          ...
          const newNotification = `  <img  className='h-11 w-11 object-cover rounded-md' src=${payment.purchase[0].product.image} alt="" />Đơn hàng mã <strong>${payment._id}</strong> đã thay đổi trạng thái <strong>${statusText}</strong>`
          ...
          <Popover
            renderPopover={
              <div className='bg-white relative shadow-md rounded-lg border border-gray-200 max-w-[400px] text-sm mr-3 font'>
                <div className='flex h-[250px] w-[350px]  '>
                  {notifications.length > 0 ? (
                    <div className='flex flex-col gap-1'>
                      {notifications.map((notification, index) => (
                        <div key={index} className='hover:bg-gray-200 w-full p-2  '>
                          {/* <Link to={path.hitoryPurchase}>
                            <div dangerouslySetInnerHTML={{ __html: notification } as any} />
                          </Link> */}
                          <div className='flex gap-2 fex-wrap'>
                            <img
                              src='https://cf.shopee.vn/file/vn-50009109-4ee7671a2d5640555a5f46d7fb82b57c'
                              alt='hihi'
                              className='h-11 w-11 object-cover rounded-md border-2 border-gray-100'
                            />

                            <div className=''>
                              Đơn hàng mã
                              <strong className='truncate mx-1'>hihi</strong>
                              đã thay đổi trạng thái
                              <strong className='truncate ml-1'>hihi</strong>
                            </div>
                          </div>
                        </div>
                      ))}
