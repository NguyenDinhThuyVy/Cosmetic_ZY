    ...
    if (paymentData && Array.isArray(paymentData.data.data) && paymentData.data.data.length > 0) {
      paymentData.data.data.forEach((payment) => {
        if (!viewedPayments.includes(payment._id)) {
          ...
          const newNotification = `Đơn hàng mã  ${payment._id} đã thay đổi trạng thái ${payment.status}`
                  ...
                  {notifications.length > 0 ? (
                    <div className='flex flex-col gap-1'>
                      {notifications.map((notification, index) => (
                        <div key={index} className='hover:bg-gray-200 w-full p-2 h-[200px] '>
                          <Link to={path.hitoryPurchase}>{notification}</Link>
                        </div>
                      ))}
                    </div>
                  ) : (
