...
function Header() {
  ...
  const [totalNotifications, setTotalNotifications] = useState(() => {
    // Lấy giá trị từ localStorage nếu có, nếu không trả về giá trị mặc định là 0
    return parseInt(localStorage.getItem('totalNotifications')) || 0
  })
    ...
    if (paymentData && Array.isArray(paymentData.data.data) && paymentData.data.data.length > 0) {
      paymentData.data.data.forEach((payment) => {
        if (!viewedPayments.includes(payment._id)) {
          ...
          localStorage.setItem('totalNotifications', totalNotifications.toString())
        }
      })
    }
                      ...
                      {notifications.map((notification, index) => (
                        <Link
                          ...
                          onClick={(event) => {
                            ...
                            handleNotificationClick() // Gọi hàm xử lý khi nhấn vào thông báo
                          }}
                          key={index}
                          className='hover:bg-gray-200 w-full p-2  '
                        >
                          <div>
