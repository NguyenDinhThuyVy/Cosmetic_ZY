...
function Header() {
  ...
  const [viewedPayments, setViewedPayments] = useState([])
  useEffect(() => {
    if (paymentData && Array.isArray(paymentData.data.data) && paymentData.data.data.length > 0) {
      paymentData.data.data.forEach((payment: any) => {
        if (!viewedPayments.includes(payment._id)) {
          // Kiểm tra xem payment._id có trong danh sách đã xem chưa
          // Nếu chưa, đây là một thay đổi mới
          setNotification(`Trạng thái thanh toán của đơn hàng ${payment._id} đã thay đổi`)
              ...
              <div className='bg-white relative shadow-md rounded-lg border border-gray-200 max-w-[400px] text-sm mr-3 font'>
                <div className='flex h-[300px] w-[300px] flex-col  items-center justify-center p-2'>
                  <img
                    ...
                    className='h-24 w-24 '
                  />
                  <div className='mt-3 capitalize'>{notification ? notification : 'Không có thông báo nào'}</div>
                </div>
              </div>
            }
          >
