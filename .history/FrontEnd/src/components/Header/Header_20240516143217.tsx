...
function Header() {
  ...
  useEffect(() => {
    if (paymentData && paymentData.data && paymentData.data.status !== previousStatus) {
      // Xử lý khi trạng thái thanh toán thay đổi
      setNotification('Trạng thái thanh toán đã thay đổi')
      setPreviousStatus(paymentData.data.status)
    }
  }, [paymentData, previousStatus])
  return (
