    ...
    if (paymentData && Array.isArray(paymentData.data.data) && paymentData.data.data.length > 0) {
      paymentData.data.data.forEach((payment) => {
        if (!viewedPayments.includes(payment._id)) {
          ...
          const newNotification = `Đơn hàng mã <strong>${payment._id}</strong> đã thay đổi trạng thái <strong>${statusText}</strong>`
                    ...
                    <div className='flex flex-col gap-1'>
                      {notifications.map((notification, index) => (
                        <div key={index} className='hover:bg-gray-200 w-full p-2  '>
                          <Link to={path.hitoryPurchase}>dangerouslySetInnerHTML={{ __html: brandData.data.data.description } as any</Link>
                        </div>
                      ))}
                    </div>
                  ) : (
