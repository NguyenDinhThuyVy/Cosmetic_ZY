...
export default function QuickView({ product }: Props) {
  ...
  const addToCart = () => {
    addToCartMutation.mutate(
      { order.buy_count: buyCount, product_id: product?._id as string },
      {
        onSuccess: (data) => {
  ...
  const buyNow = async () => {
    const res = await addToCartMutation.mutateAsync({ order.buy_count.buy_count: buyCount, product_id: product?._id as string })
