...
export default function QuickView({ product }: Props) {
  ...
  const addToCart = () => {
    addToCartMutation.mutate(
      { order.buy_count: buyCount, order.product_id: product?._id as string },
      {
        onSuccess: (data) => {
  ...
  const buyNow = async () => {
    const res = await addToCartMutation.mutateAsync({ order.buy_count: buyCount, order.product_id: product?._id as string })
